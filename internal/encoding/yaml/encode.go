...
func encodeDecls(decls []ast.Decl) (n *yaml.Node, err error) {
	...
	for _, d := range decls {
		...
		if docForNext != "" {
			lastHead.HeadComment = docForNext + lastHead.HeadComment
			docForNext = ""
		}
	}

	if docForNext != "" && lastFoot != nil {
		if !strings.HasSuffix(lastFoot.FootComment, "\n") {
			lastFoot.FootComment += "\n"
		}
		lastFoot.FootComment += docForNext[:len(docForNext)-1]
	}
