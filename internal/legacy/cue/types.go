...
func newErrValue(v Value, b *bottom) Value {
	if v.v == nil {
		return newValueRoot(v.ctx(), b)
	}
	parent := v.v.Parent
	node := &adt.Vertex{Parent: parent, Label: v.v.Label, Value: b}
	node.AddConjunct(adt.MakeConjunct(nil, b))
	return Value{v.idx, node}
}

func newVertexRoot(ctx *context, x *adt.Vertex) Value {
	eval.Evaluate(ctx.Runtime, x)
	return Value{ctx.index, x}
}
...
func (v Value) Syntax(opts ...Option) ast.Node {
	...
	// }
	if o.raw {
		// TODO: export.Conjuncts()
		n, _ := export.Raw.Def(v.idx.Runtime, v.v)
		return n
	}
	if o.concrete || o.final {
		f, err := export.Simplified.Vertex(v.idx.Runtime, v.v)
		if err != nil {
			return nil
		}
		return f
	}
	n, _ := export.Simplified.Def(v.idx.Runtime, v.v)
	return n
}
...
func (v Value) checkKind(ctx *context, want kind) *bottom {
	...
	k := x.Kind()
	if want != bottomKind && k&want == bottomKind {
...
func (v Value) Elem() (Value, bool) {
	...
	v.v.MatchAndInsert(ctx.opCtx, x)
...
func (v Value) structValOpts(ctx *context, o options) (structValue, *bottom) {
	...
	if o.omitHidden || o.omitOptional || o.omitDefinitions {
		...
		for _, a := range obj.Arcs {
			f |= a.Label
			if a.Label.IsDef() && (o.omitDefinitions || o.concrete) {
				needFilter = true
				break
			}
		}
	...
	return structValue{ctx, v, obj, obj.Arcs}, nil
}
...
func (s *Struct) Field(i int) FieldInfo {
	...
	opt := false
	return FieldInfo{str, i, v, a.Label.IsDef(), opt, a.Label.IsHidden()}
}
...
func (v Value) LookupDef(name string) Value {
	...
	return newErrValue(v, ctx.mkErr(v.v, "defintion %q not found", name))
}
...
func (v Value) Unify(w Value) Value {
	...
	n.Add(v.v)
	n.Add(w.v)
	// if len(v.v.Conjuncts) > 0 {
	// 	for _, c := range v.v.Conjuncts {
	// 		n.AddConjunct(c)
	// 	}
	// } else {
	// 	for _, a := range v.v.Arcs {
	// 		sub, _ := n.GetArc(a.Label)
	// 		sub.AddConjunct(adt.MakeConjunct(nil, a.Value))
	// 	}
	// }
	// if len(w.v.Conjuncts) > 0 {
	// 	for _, c := range w.v.Conjuncts {
	// 		n.AddConjunct(c)
	// 	}
	// } else {
	// 	for _, a := range w.v.Arcs {
	// 		sub, _ := n.GetArc(a.Label)
	// 		sub.AddConjunct(adt.MakeConjunct(nil, a.Value))
	// 	}
	// }
	ctx := v.idx.newContext()
	ctx.opCtx.Unify(ctx.opCtx, n)
	return Value{v.idx, n}
}
...
func (v Value) Expr() (Op, []Value) {
	...
	case *binaryExpr:
		a = append(a, remakeValue(v, c.Env, x.X))
		a = append(a, remakeValue(v, c.Env, x.Y))
		op = x.Op
	case *unaryExpr:
		a = append(a, remakeValue(v, c.Env, x.X))
		op = x.Op
	case *boundExpr:
		a = append(a, remakeValue(v, c.Env, x.Expr))
		op = x.Op
	case *boundValue:
		a = append(a, remakeValue(v, c.Env, x.Value))
		op = x.Op
	case *adt.Conjunction:
		// pre-expanded unification
		for _, conjunct := range x.Values {
			a = append(a, remakeValue(v, c.Env, conjunct))
		}
		op = AndOp
	case *disjunction:
		...
		for _, disjunct := range x.Values {
			...
			// }
			count++
			a = append(a, remakeValue(v, c.Env, disjunct.Val))
		}
		if count > 1 {
			op = adt.OrOp
		}
	case *interpolation:
		for _, p := range x.Parts {
			a = append(a, remakeValue(v, c.Env, p))
		}
		op = InterpolationOp
	...
	case *selectorExpr:
		...
		a = append(a, remakeValue(v, c.Env, &adt.String{
	...
	case *indexExpr:
		a = append(a, remakeValue(v, c.Env, x.X))
		a = append(a, remakeValue(v, c.Env, x.Index))
		op = IndexOp
	case *sliceExpr:
		a = append(a, remakeValue(v, c.Env, x.X))
		a = append(a, remakeValue(v, c.Env, x.Lo))
		a = append(a, remakeValue(v, c.Env, x.Hi))
		op = SliceOp
	case *callExpr:
		a = append(a, remakeValue(v, c.Env, x.Fun))
		for _, arg := range x.Args {
			a = append(a, remakeValue(v, c.Env, arg))
		}
		op = CallOp
	case *customValidator:
		a = append(a, remakeValue(v, c.Env, x.Builtin))
		for _, arg := range x.Args {
			a = append(a, remakeValue(v, c.Env, arg))
		}
		op = CallOp
