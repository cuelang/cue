...
func evalExpr(ctx *context, scope *adt.Vertex, expr ast.Expr) evaluated {
	...
	x, err := compile.Expr(&compile.Config{Scope: scope}, ctx.opCtx.Runtime, expr)
	if err != nil {
		return &adt.Bottom{Err: err}
	}
	v := &adt.Vertex{Parent: scope.Parent, Label: scope.Label}
	v.AddConjunct(adt.MakeConjunct(&adt.Environment{Vertex: scope}, x))
	return ctx.vertex(v)
}
...
func (inst *Instance) Eval(expr ast.Expr) Value {
	ctx := inst.newContext()
	result := evalExpr(ctx, inst.root, expr)
