...
test: _#bashWorkflow & {
	...
	_#gerrit: {
		...
		_#setCodeReview: {
			...
			res: #"""
			\#(_#curl) "Content-Type: application/json" --request POST --data '\#(encjson.Marshal(#args))' -b ~/.gitcookies https://cue-review.googlesource.com/a/changes/$(basename $(dirname $GITHUB_REF))/revisions/$(basename $GITHUB_REF)/review
			"""#
		}
	}
}
