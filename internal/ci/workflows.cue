...
test: json.#Workflow & {
	...
	jobs: {
		...
		delete_build_branch: {
			...
			steps: [
				_#step & {
					run: "echo git push origin :$GITHUB_REF"
				},
			]
		}
	}
...
_#goGenerate: _#step & {
	...
	if: "matrix.go-version == '\(_#codeGenGo)' && matrix.os != 'windows-latest'"
}
