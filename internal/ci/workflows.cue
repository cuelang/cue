...
repository_dispatch: _#bashWorkflow & {
	...
	_#copybaraCmd: {
		...
		#"""
		cd _scripts
		docker run --rm -v $PWD/cache:/root/copybara/cache -v $PWD:/usr/src/app --entrypoint="" -it \#(_#cueckooCopybaraImage) bash -c " \
			set -eu; \
			...
			java -jar /opt/copybara/copybara_deploy.jar migrate copy.bara.sky \#(#cmd); \
			"
		"""#
	}
