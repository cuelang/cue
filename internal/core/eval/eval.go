...
func (e *Evaluator) evalVertex(c *adt.OpContext, v *adt.Vertex, state adt.VertexStatus) *nodeShared {
	...
	if v.Parent != nil {
		...
		// ignore = v.Parent.Ignore
	}
	saved := *v

	if !v.Label.IsInt() && v.Parent != nil && !ignore {
		// Visit arcs recursively to validate and compute error.
		if _, err := verifyArc(c, v.Label, v, v.ClosedX); err != nil {
...
func (n *nodeContext) addValueConjunct(env *adt.Environment, v adt.Value, id adt.CloseInfo) {
	...
	if x, ok := v.(*adt.Vertex); ok {
		if m, ok := x.BaseValue.(*adt.StructMarker); ok {
			...
			n.aStructID = id
			if m.NeedClose {
				n.node.ClosedX = true // TODO: remove.
		...
		case *adt.StructMarker:
			...
			n.node.AddStruct(adt.StructInfo{
				StructLit: s,
				Env:       env,
				CloseInfo: id,
			})
...
func (n *nodeContext) addStruct(
	...
	sPos := len(n.node.Structs)
	parent := n.node.AddStruct(adt.StructInfo{
		StructLit: s,
		Env:       childEnv,
		CloseInfo: closeInfo,
		Disable:   true,
	...
	n.node.Structs[sPos].Disable = false
...
outer:
	...
	for _, l := range n.lists {
		...
		s.Init()
		info := n.node.AddStruct(adt.StructInfo{
			StructLit: s,
			Env:       l.env,
			CloseInfo: l.id,
		})
