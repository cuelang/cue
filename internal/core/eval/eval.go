...
func (e *Evaluator) evalVertex(c *adt.OpContext, v *adt.Vertex, state adt.VertexStatus, accept adt.Acceptor) *nodeShared {
	...
	for i := 0; ; i++ {
		...
		*v = saved
		...
		if closedInfo != nil {
			canopy := make([]CloseDef, len(closedInfo.Canopy))
			copy(canopy, closedInfo.Canopy)
			for i := range canopy {
...
func (n *nodeContext) updateClosedInfo() {
	...
	for _, a := range n.node.Arcs {
		if Debug {
			fmt.Printf("%d(%s): %v\n",
				a.Label, a.Label.SelectorString(ctx), c.Canopy)
		}
		...
		} else if err := c.verifyArcAllowed(n.ctx, a.Label, a); err != nil {
...
outer:
	...
	case adt.Resolver:
		...
		if arc.Label.IsDef() { // should test whether closed, not isDef?
			closeID = closedInfo(n.node).InsertDefinition(v.CloseID, x)
