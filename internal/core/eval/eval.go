	...
	if v.Value == nil {
		...
		*v = save
		...
		if c.IsTentative() && isStruct(v) {
			...
			arcs := v.Arcs
			*v = save
			return &adt.Vertex{Value: &adt.StructMarker{}, Arcs: arcs}
		}
		// *v = save // DO NOT ADD.
		...
		if isEvaluating(n.node) {
			...
			if !n.done() { // && !ctx.IsTentative() {
				...
				for _, d := range n.dynamicFields {
					x, _ := ctx.Evaluate(d.env, d.field.Key)
					b, _ := x.(*adt.Bottom)
...
func updateCyclic(c adt.Conjunct, cyclic bool, deref *adt.Vertex) adt.Conjunct {
	...
	} else if deref == nil && env.Cyclic == cyclic {
		return c
	} else {
		e := *env
	...
	if x, ok := v.(*adt.Vertex); ok {
		...
		if len(x.Conjuncts) > 0 {
			...
			for _, c := range x.Conjuncts {
				n.addExprConjunct(updateCyclic(c, cyclic, nil), 0, false) // Pass from eval
			}
		} else {
		...
		n.addValueConjunct(env, x.Value)
		return
