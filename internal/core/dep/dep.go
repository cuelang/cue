...
type VisitFunc func(Dependency) bool
...
func Visit(c *adt.OpContext, n *adt.Vertex, f VisitFunc) {
	visit(c, n, f, false)
}
...
func VisitAll(c *adt.OpContext, n *adt.Vertex, f VisitFunc) {
	visit(c, n, f, true)
}
...
func visit(c *adt.OpContext, n *adt.Vertex, f VisitFunc, all bool) {
	if c == nil {
		panic("nil context")
	}
	v := visitor{
	...
	defer func() {
		// if err := recover(); err != nil && err != aborted {
		// 	panic(err)
		// }
	}()
...
func (c *visitor) markResolver(env *adt.Environment, r adt.Resolver) {
	...
	if ref, _ := c.ctxt.Resolve(env, r); ref != nil {
		if ref != c.node {
			d := Dependency{Node: ref, Reference: r}
			if !c.visit(d) {
				panic(aborted)
			}
		}
