...
var lenBuiltin = &adt.Builtin{
	Name:   "len",
	Params: []adt.Kind{adt.BytesKind | adt.StringKind | adt.ListKind | adt.StructKind},
	Result: adt.IntKind,
	Func: func(c *adt.OpContext, args []adt.Value) adt.Expr {
		...
		default:
			return c.NewErrf("invalid argument type %v", v.Kind())
		}
	},
}
...
var orBuiltin = &adt.Builtin{
	...
	Func: func(c *adt.OpContext, args []adt.Value) adt.Expr {
		...
		v.AddConjunct(adt.MakeConjunct(nil,
			&adt.Disjunction{Values: d, HasDefaults: false},
		))
		c.Unify(c, v)
		return v
	},
