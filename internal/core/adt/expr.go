...
func (x *NodeLink) Kind() Kind                   { return x.Node.Kind() }
	...
	for i, a := range args {
		if x.Params[i] != BottomKind {
			...
			if k := kind(a); x.Params[i]&k == BottomKind {
				// TODO: make this a permanent error if the value did not
				// originate form a reference.
				c.addErrf(IncompleteError, pos(a),
