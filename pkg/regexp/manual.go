...
// containing the successive. The integer argument n indicates the maximum
// number of matches to return for n >= 0, or all matches otherwise. It returns
// bottom for no match.
...
// FindSubmatch returns a slice strings holding the text of the leftmost match
...
// MapSubmatch is like FindSubmatch, but returns a map with the named used
// in capturing groups.
func MapSubmatch(pattern, s string) (map[string]string, error) {
	re, err := regexp.Compile(pattern)
	if err != nil {
		return nil, err
	}
...
// MapAllSubmatch is like FindAllSubmatch, but returns a map with the named used
// in capturing groups.
func MapAllSubmatch(pattern, s string, n int) ([]map[string]string, error) {
	re, err := regexp.Compile(pattern)
	if err != nil {
		return nil, err
	}
