...
func init() {
	...
	task.Register("tool/file.Dir", newDirCmd)
...
func newDirCmd(v cue.Value) (task.Runner, error)    { return &cmdDir{}, nil }
...
func (c *cmdDir) Run(ctx *task.Context, v cue.Value) (res interface{}, err error) {
	dir, err := ioutil.ReadDir(lookupStr(v, "dir"))
	if err != nil {
		return nil, err
	}
