...
var constraints = []*constraint{
	...
	p1("type", func(n cue.Value, s *state) {
		...
		set := func(n cue.Value) {
			...
			case "null":
				types |= cue.NullKind
				s.setType(n, nullType, "null")
				// TODO: handle OpenAPI restrictions.
			case "boolean":
				types |= cue.BoolKind
				s.setType(n, boolType, "bool")
			case "string":
				types |= cue.StringKind
				s.setType(n, stringType, "string")
			case "number":
				types |= cue.NumberKind
				s.setType(n, numType, "number")
			case "integer":
				types |= cue.IntKind
				s.setType(n, numType, "number")
			...
			case "array":
				types |= cue.ListKind
				c := ast.NewList(&ast.Ellipsis{})
				setPos(c, n)
				s.types[arrayType].typ = c
			case "object":
				types |= cue.StructKind
				c := &ast.StructLit{}
				setPos(c, n)
				s.types[objectType].typ = c
	...
	p1("pattern", func(n cue.Value, s *state) {
		...
		pattern := &ast.UnaryExpr{Op: token.MAT, X: s.string(n)}
		s.add(n, stringType, pattern)
	}),
	...
	p1("minLength", func(n cue.Value, s *state) {
		...
		x := ast.NewCall(ast.NewSel(strings, "MinRunes"), min)
		s.add(n, stringType, x)
	}),
	...
	p1("maxLength", func(n cue.Value, s *state) {
		...
		max := s.number(n)
		strings := s.addImport(n, "strings")
		x := ast.NewCall(ast.NewSel(strings, "MaxRunes"), max)
		s.add(n, stringType, x)
	}),
	...
	p1("multipleOf", func(n cue.Value, s *state) {
		...
		c := ast.NewCall(ast.NewSel(math, "MultipleOf"), multiple)
		s.add(n, numType, c)
	}),
	...
	p1("uniqueItems", func(n cue.Value, s *state) {
		...
		if s.boolValue(n) {
			list := s.addImport(n, "list")
			x := ast.NewCall(ast.NewSel(list, "UniqueItems"))
			s.add(n, arrayType, x)
		}
	}),
}
