...
func (d *decoder) decode(filename string, inst *cue.Instance) *ast.File {
	...
	// TODO: more attributes: title, description, examples
	...
	if len(d.definitions) > 0 {
		...
		// 	Token: token.ISA,
		// 	Value: &ast.StructLit{Elts: d.definitions},
		// }
		// ast.SetRelPos(defs, token.NewSection)
		a = append(a, d.definitions...)
	}

	file := &ast.File{Decls: a}
...
func (s *state) finalize() (e ast.Expr) {
	...
	for _, n := range s.types {
		...
		case "null":
			...
			add(ast.NewIdent("null")) // Use __null
		case "boolean":
			add(ast.NewIdent("bool")) // Use __bool
		case "string":
			add(ast.NewIdent("string")) // Use __string
		case "number":
			add(ast.NewIdent("number")) // Use __number
		case "integer":
			add(ast.NewIdent("int")) // Use __int
		case "array":
