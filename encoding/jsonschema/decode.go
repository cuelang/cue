...
func (c *constraintInfo) setType(n cue.Value, x string) {
	c.typ = ast.NewIdent(x)
	setPos(c.typ, n)
...
func (s *state) setType(n cue.Value, t coreType, x string) {
	s.types[t].setType(n, x)
}
...
func (s *state) hasConstraints() bool {
	hasConjuncts := len(s.all.constraints) > 0
	for i := 0; !hasConjuncts && i < len(s.types); i++ {
		t := s.types[i]
		hasConjuncts = len(t.constraints) > 0
	}
	return hasConjuncts ||
		len(s.patterns) > 0 ||
		s.title != "" ||
...
func (s *state) finalize() (e ast.Expr) {
	...
	for i, t := range s.types {
		...
		if len(t.constraints) > 0 {
			...
			case cue.FloatKind:
				x = ast.NewIdent("number")
			case cue.ListKind:
				x = ast.NewList(&ast.Ellipsis{})
			case cue.StructKind:
				x = ast.NewStruct(&ast.Ellipsis{})
			}
			if x != nil {
				disjuncts = append(disjuncts, x)
			}
