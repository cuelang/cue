...
func (p *protoConverter) qualImportPath() string {
	s := p.importPath()
...
func (p *protoConverter) resolveTopScope(pos scanner.Position, name string, options []*proto.Option) ast.Expr {
	for i := 0; i < len(name); i++ {
		...
		if m, ok := p.scope[0][name[:i]]; ok {
			if m.pkg != nil {
				p.imported[m.pkg.qualImportPath()] = true
			}
			expr := m.cue()
			ast.SetPos(expr, p.toCUEPos(pos))
			return expr
	...
	for {
		for k := range imp.symbols {
			...
			if _, ok := p.scope[0][ref]; !ok {
				...
				if imp.qualImportPath() == p.qualImportPath() {
					pkg = nil
					...
					f = func() ast.Expr {
						ident := &ast.Ident{
							Name: imp.shortName(),
							Node: ast.NewImport(nil, imp.qualImportPath()),
						}
