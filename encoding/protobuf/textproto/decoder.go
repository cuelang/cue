...
import (
	...
	"cuelang.org/go/encoding/protobuf/internal"
...
// A Decoder caches conversions of cue.Value between calles to its methods.
type Decoder struct {
...
func (d *decoder) parseSchema(schema cue.Value) *mapping {
	...
	for i.Next() {
		...
		case internal.List, internal.Map:
			e, _ := i.Value().Elem()
...
func (d *decoder) decodeMsg(m *mapping, n []*pbast.Node) ast.Expr {
	...
	for _, x := range n {
		...
		case internal.List:
			...
			if len(x.Values) == 1 || f.ValueType == internal.Message {
				v := d.decodeValue(f, x)
				if value == nil {
					if cg := addComments(x.PreComments...); cg != nil {
						cg.Doc = true
...
func (d *decoder) decodeValue(f *fieldInfo, n *pbast.Node) (x ast.Expr) {
	if f.ValueType == internal.Message {
	...
	case internal.Int, internal.Float:
		s := v.Value
		switch s {
		case "inf", "nan":
