...
func (e *encoder) rewrite(schema cue.Value, expr ast.Expr) (x ast.Expr) {
	...
	case *ast.BasicLit:
		...
		t, err := a.String(1)
		if err != nil {
			e.addErr(errors.Promote(err, "encoder"))
			break
		}
		...
		default:
			if r, _ := utf8.DecodeRuneInString(t); !unicode.In(r, unicode.Upper) {
				break
			}
...
func matchByInt(v cue.Value, val int64, x *ast.BasicLit) bool {
	switch op, a := v.Expr(); op {
	case cue.OrOp, cue.AndOp:
		for _, v := range a {
			if matchByInt(v, val, x) {
				return true
			}
		}
	...
	default:
		...
		x.Kind = token.STRING
		x.Value = literal.String.Quote(sel.String()[1:])
		return true
	}
