...
// Rewrite modifies expr, interpreting it in terms of the given schema
// according to the protocol buffer to JSON mapping defined in the
// protocol buffer spec.
//
// It mostly bases the mapping on the underlying Go type.
//
...
//  _:         left as is.
//
// Rewrite is idempotent, calling it multiples times on an expression gives the
// same result.
//
// NOTE: the signature of this call may change to accept options.
func Rewrite(schema cue.Value, expr ast.Expr) (ast.Expr, error) {
	var r rewriter
	x := r.rewrite(schema, expr)
	return x, r.errs
}

// Rewrite modifies file, interpreting it in terms of the given schema
// according to the protocol buffer to JSON mapping defined in the
// protocol buffer spec.
//
// See Rewrite for details.
func RewriteFile(schema cue.Value, file *ast.File) error {
	var r rewriter
	r.rewriteDecls(schema, file.Decls)
	return r.errs
}
