...
func (b *builder) coreSchema(name string) *oaSchema {
	...
	default:
		...
		// safely assume there wer no disjunctions etc. In structural mode this is
		// the only chance we get to set certain properties.
		if len(b.values) == 1 {
	...
	case cue.ListKind:
		if b.items != nil {
			...
			schema := b.items.schema(nil, "*", b.items.values[0])
	...
	case cue.StructKind:
		...
		if b.items != nil && len(b.items.values) == 1 {
			// schema := b.items.coreSchema("*")
			schema := b.items.schema(nil, "*", b.items.values[0])
