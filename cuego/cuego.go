...
// MustAddConstraints is like AddConstraints, but panics if there is an error.
func MustAddConstraints(x interface{}, constraints string) {
	if err := AddConstraints(x, constraints); err != nil {
		panic(err)
	}
}

// AddConstraints associates the given CUE constraints with the type of x or
// reports an error if the constraints are invalid or not compatible with x.
//
// AddConstraints works across package boundaries and is typically called in the
// package defining the type. Use a Context to apply constraints locally.
func AddConstraints(x interface{}, constraints string) error {
	return DefaultContext.AddConstraints(x, constraints)
}
...
// Update sets previously undefined values in x that can be uniquely determined
// form the constraints defined on the type of x such that validation passes,
// or returns an error, without modifying anything, if this is not possible.
//
// Update does a JSON round trip. This means that data not preserved in such
// a round trip, such as the location name of a time.Time, is lost after
// a successful update.
func Update(x interface{}) error {
	return DefaultContext.Update(x)
}
...
// a round trip, such as the location name of a time.Time, is lost after
// a successful update.
func (c *Context) Update(x interface{}) error {
	a := c.load(x)
	v, err := fromGoValue(x)
	if err != nil {
		return err
...
// TODO: should we require that AddConstraints be defined on exported,
// named types types only?

// AddConstraints associates the given CUE constraints with the type of x or
// reports an error if the constraints are invalid or not compatible with x.
func (c *Context) AddConstraints(x interface{}, constraints string) error {
