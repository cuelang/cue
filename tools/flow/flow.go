...
type Config struct {
	...
	// Allow tasks to be defined outside of the Root. Such tasks will only be
	// included in the workflow if any of its fields is referenced by any of the
	// tasks defined within Root.
	InferNonRootTasks bool
...
// This list may grow over time when running.
...
//
// The following state diagram applies to the current implementation.
//
//   Waiting  →  Running
//       ↘︎        ↙︎
//       Terminated
//
// The foreseen state diagram, which allows for related generated tasks to
// be pooled and for continuously running tasks for serving applications.
//
//          Ready
//       ↗︎        ↘︎
//   Waiting  ←  Running
...
const (
	...
	//
	// NOTE: this state is currently unused, as a goroutine is spawned for a
	// task as soon as it is found to be in the ready state, causing a task
	// to be marked as running. This state may be used in the future, though,
	// if a series of related tasks are started using a worker pattern.
	Ready
...
func (t *Task) Controller() *Controller {
	return t.c
}
