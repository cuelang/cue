...
func (c *Controller) findImpliedTask(n *adt.Vertex) *Task {
	for ; n != nil; n = n.Parent {
		t, ok := c.nodes[n]
		if ok || !c.cfg.InferNonRootTasks {
			return t
		}
...
func (c *Controller) markTaskDependencies(t *Task, n *adt.Vertex) {
	dep.VisitAll(c.opCtx, n, func(d dep.Dependency) bool {
		depTask := c.findImpliedTask(d.Node)
		if depTask != nil {
