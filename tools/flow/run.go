...
// This file contains logic for launching tasks.
//
		...
		for _, t := range c.tasks {
			...
			go func(t *Task) {
				if err := t.r.Run(t, nil); err != nil {
					t.err = errors.Promote(err, "task error")
				}
...
func (c *Controller) updateHook(t *Task) {
...
func (c *Controller) updateTaskValue(t *Task) {
	...
	for _, dep := range t.depTasks {
		if dep.seqNum > required {
			required = dep.seqNum
		}
	}

	if t.seqNum == required {
		return
	}
