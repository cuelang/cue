...
Run 'cue help cmd' to show available commands.`,
	...
	// TODO: this is an alternative to the above code. Instead of changing cmd,
	// though, a better behavior seems to deprecate cmd and replace it with run.
	//
	// if _, ok := sub[name]; ok {
	// 	if len(args) == 1 {
	// 		_, err := findCustomCommands(cmd, sub)
...
func (c *commandIndex) parseCommand(spec *subSpec, pkgName, name string, v cue.Value) {
	...
	sub := &cobra.Command{
		RunE: mkRunE(c.cmd, func(cmd *Command, args []string) error {
			inst, err := buildToolPkg(cmd, pkgName, args, tools)
