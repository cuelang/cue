...
var commandsHelp = &cobra.Command{
	...
	Long: `Commands define actions on instances. For example, they may specify
...
Each command consists of one or more tasks. A task may load or
write a file, consult a user on the command line, fetch a web page,
and so on. Each task has inputs and outputs. Outputs are typically
...
In this example we declare the "prompter" command which has four tasks....
