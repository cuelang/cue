...
// Modes:
// multiple packages
// a files package, using -l also with data files.
// stream plus package
//
// In all cases, the -m flag can be used to mix in packages with all.
type buildPlan struct {
	cmd   *Command
	insts []*build.Instance

	// If orphanFiles are mixed with CUE files and/or if rooting flags are used,
...
func parseArgs(cmd *Command, args []string, cfg *load.Config) (*buildPlan, error) {
	...
	for _, b := range builds {
		...
		// XXX: Check if rooting flags are specified.
		// p.user = append(user, b)
		for _, f := range b.OrphanedFiles {
			...
			case build.Protobuf:
				p.orphanedSchema = append(p.orphanedSchema, f)
			case build.YAML, build.JSON, build.TXT:
