...
specified packages. By default, import only looks for JSON and
YAML files (see the "filetypes" help topic for more info). This
default behavior can be overridden by specifying one of the
following modes:
   ...
   auto       Interpret JSON and YAML as data, JSON Schema, or
              OpenAPI, depending on existing fields.
...
func runImport(cmd *Command, args []string) (err error) {
	filter := `\.(json|yaml|yml|jsonl|ldjson)$`
	mode := ""
	if len(args) > 0 {
		switch args[0] {
		case "proto":
		...
		case "yaml":
			...
			args = args[1:]
			// TODO:
			//  - auto (between openapi, json, yaml, and json schema)
			//  - openapi (filter json, yaml and non-package cue)
			//  - jsonschema (filter json, yaml and non-package cue)
		}
	}
	if s := flagGlob.String(cmd); s == "" {
		// Set a default file filter to only include json and yaml files
		_ = cmd.Flags().Set(string(flagGlob), filter)
	}
...
func dataMode(cmd *Command, b *buildPlan) error {
