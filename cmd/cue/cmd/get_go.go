...
The command "cue get go" is like "get go", but converts the retrieved Go
...
It is save for users to add additional files to the generated directories,
...
Go structs are converted to cue structs adhering to the following conventions:
	...
	- slices and arrays convert to CUE lists, except when the element type is
	  ...
	  In the case of arrays, the length of the CUE value is constraint
	  accordingly, when possible.

	- Maps translate to a CUE struct, where all elements are constraint to
...
that are not otherwise expressable in Go. The package name for these CUE files
...
func getName(name string, tag string) string {
	...
	for _, s := range []string{"cue", "json", "yaml"} {
