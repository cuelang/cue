...
type Selector interface {
	// String resturns the CUE representation of a selector.
	String() string
...
func ParsePath(s string) Path {
	...
	if err != nil {
		return []Selector{pathError{errors.Promote(err, "invalid path")}}
	}

	return toSelectors(expr)
}
...
func toSelectors(expr ast.Expr) Path {
	switch x := expr.(type) {
	case *ast.Ident:
...
func identSelector(label ast.Label) Selector {
	...
	case *ast.Ident:
		if isHiddenOrDefinition(x.Name) {
			return definitionSelector(x.Name)
		}
		return stringSelector(x.Name)
