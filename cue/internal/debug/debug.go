...
func WriteNode(w io.Writer, i adt.StringIndexer, n adt.Node) {
	p := printer{Writer: w, index: i}
	p.node(n)
}

func NodeString(i adt.StringIndexer, n adt.Node) string {
	b := &strings.Builder{}
	p := printer{Writer: b, index: i}
	p.node(n)
	return b.String()
}
...
func (w *printer) node(n adt.Node) {
	...
	case *adt.Field:
		...
		w.string(s)
		w.string(":")
		if x.Label.IsDef() && !strings.HasPrefix(s, "#") {
			w.string(":")
		}
		w.string(" ")
		w.node(x.Value)
