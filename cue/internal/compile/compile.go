...
type compiler struct {
	...
	// TODO:
	// dynamic specifies a node in which to look up unresolved references. This
...
func (c *compiler) path() []string {
	a := make([]string, len(c.stack))
	for i, f := range c.stack[1:] {
		a[i] = f.label.labelString()
	}
	return a
}
...
func (c *compiler) pushScope(n labeler, id ast.Node) *frame {
	upCount := int32(0)
	if n != nil {
		upCount++
	} else if _, ok := id.(*ast.File); !ok {
		panic("label may only be nil for *ast.File")
	}
...
func (c *compiler) compileFiles(a ...*ast.File) adt.Arc { // Or value?
	...
	env := &adt.Environment{} // runtime: c.runtime

	for _, file := range a {
		c.pushScope(nil, file) // File scope
		v := &adt.StructLit{Src: file}
		c.addDecls(v, file.Decls)
		res.Conjuncts = append(res.Conjuncts, adt.EnvExpr{Env: env, X: v})
		c.popScope()
	}
...
// func (c *compiler) compileExpr(env *environment, n ast.Expr) expression {
...
func (c *compiler) resolve(n *ast.Ident) adt.Expr {
	...
	if f, ok := n.Scope.(*ast.Field); ok {
		...
		if f, ok := n.Node.(*ast.Field); ok {
			e := c.lookupAlias(k, f.Label.(*ast.Alias).Ident) // mark as used
			if e.expr != nil {
...
func (c *compiler) decl(d ast.Decl) adt.Decl {
	...
	case *ast.Field:
		...
		if a, ok := lab.(*ast.Alias); ok {
			...
			default:
				e.expr = c.expr(dynamicLabelExpr{lab}, a.Expr)
			}
		...
		c.stack[len(c.stack)-1].field = x
		value := c.expr((*fieldLabel)(x), x.Value)
		c.stack[len(c.stack)-1].field = nil
		...
		case *ast.ListLit:
			...
			return &adt.BulkOptionalField{
				Src:    x,
				Filter: c.expr((*bulkLabelExpr)(l), elem),
				Value:  value,
			}
		...
		case *ast.Interpolation: // *ast.ParenExpr,
			...
			return &adt.DynamicField{
				Src:   x,
				Key:   c.expr((*interpolationLabel)(l), l),
				Value: value,
			}
		}
	...
	case *ast.Alias:
		...
		a := aliasEntry{
			source: x,
			expr:   c.expr((*deprecatedAliasScope)(x), x.Expr),
		}
	...
	case *ast.EmbedDecl: // Deprecated
		return c.expr(embedLabel{}, x.Expr)

	case *ast.Ellipsis:
		return &adt.Ellipsis{
			Src:   x,
			Value: c.expr((*ellipsisLabel)(x), x.Type),
		}
	...
	for _, v := range x.Clauses {
		...
		case *ast.ForClause:
			...
			y := &adt.ForClause{
				...
				Src:    c.expr((*forSource)(x), x.Source),
			}
			cur = y
			c.pushScope((*forScope)(x), v)
			defer c.popScope()
			next = &y.Dst
		...
		case *ast.IfClause:
			y := &adt.IfClause{
				Src:       x,
				Condition: c.expr(nil, x.Condition),
			}
			cur = y
			next = &y.Dst
		...
		case *ast.LetClause:
			y := &adt.LetClause{
				Src: x,
				Scope: &adt.StructLit{
					Decls: []adt.Decl{&adt.Field{
						Label: c.label(x.Ident),
						Value: c.expr((*letCond)(x), x.Expr),
					}},
				},
			}
			cur = y
			c.pushScope((*letScope)(x), v)
			defer c.popScope()
			next = &y.Dst
		}
...
func (c *compiler) expr(parent labeler, expr ast.Expr) adt.Expr {
	switch n := expr.(type) {
	case nil:
		return nil
	case *ast.Ident:
	...
	case *ast.ListLit:
		...
		if ellipsis != nil {
			d := &adt.Ellipsis{
				Src:   ellipsis,
				Value: c.expr(parent, ellipsis.Type),
			}
	...
	case *ast.IndexExpr:
		return &adt.IndexExpr{
			Src:   n,
			X:     c.expr(parent, n.X),
			Index: c.expr(parent, n.Index),
		}

	case *ast.SliceExpr:
		slice := &adt.SliceExpr{Src: n, X: c.expr(parent, n.X)}
		if n.Low != nil {
			slice.Lo = c.expr(parent, n.Low)
		}
		if n.High != nil {
			slice.Hi = c.expr(parent, n.High)
		}
		return slice
	...
	case *ast.Interpolation:
		...
		if len(n.Elts) == 1 {
			return c.expr(parent, n.Elts[0])
		}
		lit := &adt.Interpolation{Src: n, Kind: adt.StringKind}
		...
		for i := 0; i < len(n.Elts); i += 2 {
			...
			if i+1 < len(n.Elts) {
				lit.Parts = append(lit.Parts, c.expr(parent, n.Elts[i+1]))
			}
			prefix = ")"
			prefixLen = 1
		}
	...
	case *ast.ParenExpr:
		return c.expr(parent, n.X)

	case *ast.CallExpr:
		call := &adt.CallExpr{Src: n, Fun: c.expr(parent, n.Fun)}
		for _, a := range n.Args {
			call.Args = append(call.Args, c.expr(parent, a))
		}
		return call
	...
	case *ast.BinaryExpr:
		switch n.Op {
		case token.OR:
			d := &adt.Disjunction{Src: n}
			c.addDisjunctionElem(parent, d, n.X, false)
			c.addDisjunctionElem(parent, d, n.Y, false)
			return d
		...
		default:
			...
			return &adt.BinaryExpr{
				...
				X:   c.expr(parent, n.X),   // left
				Y:   c.expr(parent, n.Y),   // right
			} // )
		}
...
func (c *compiler) addDisjunctionElem(parent labeler, d *adt.Disjunction, n ast.Expr, mark bool) {
	switch x := n.(type) {
	case *ast.BinaryExpr:
		if x.Op == token.OR {
			c.addDisjunctionElem(parent, d, x.X, mark)
			c.addDisjunctionElem(parent, d, x.Y, mark)
			return
		}
	case *ast.UnaryExpr:
		if x.Op == token.MUL {
	...
	d.Values = append(d.Values, adt.Disjunct{Val: c.expr(parent, n), Default: mark})
}
