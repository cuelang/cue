...
func TestBasicRewrite(t *testing.T) {
	testCases := []testCase{{
		...
		out: `<0>{c1: true, ` +
			...
			`e3: _|_((!="a" & <5):conflicting values !="a" and <5 (incompatible types string and number))}`,
	}, {
		desc: "arithmetic",
		in: `
			i1: 1 & int
		...
		in: `
			...
			f: bool
			`,
		out: `<0>{a: 1, b: 1, c: 1.0, d: _|_((int & float):conflicting values int and float (incompatible types int and float)), e: "4", f: true}`, // TODO: eliminate redundancy
	}, {
		...
		in: `
			...
			e5: [1, 2, 4, 8] & [1, 2, ...>=4 & <=5]
			`,
		out: `<0>{list: [1,2,3], index: 2, unify: [1,2,3], e: _|_(([] & 4):conflicting values [] and 4 (incompatible types list and int)), e2: _|_("d":invalid list index "d" (type string)), e3: _|_(-1:invalid list index -1 (index must be non-negative)), e4: [1,2,4,_|_((<=5 & 8):invalid value 8 (out of bound <=5))], e5: [1,2,4,_|_((<=5 & 8):invalid value 8 (out of bound <=5))]}`,
	}, {
		...
		in: `
			...
			e: {a:3}
			`,
		out: "<0>{o1: <1>{a: 1, b: 2}, o2: <2>{a: 1, b: 2}, o3: <3>{a: 1, b: 2}, o4: <4>{a: 1, b: 2}, e: _|_((1 & <5>{a: 3}):conflicting values 1 and {a: 3} (incompatible types int and struct))}",
	}, {
		desc: "disjunctions",
		in: `
			o1: 1 | 2 | 3
		...
		in: `
			...
			m: -false
		`,
		out: `<0>{i: int, j: 3, s: string, t: "s", e: _|_((int & string):conflicting values int and string (incompatible types int and string)), e2: _|_((1 & string):conflicting values 1 and string (incompatible types int and string)), b: _|_(!int:invalid operation !int (! int)), p: _|_(+true:invalid operation +true (+ bool)), m: _|_(-false:invalid operation -false (- bool))}`,
	}, {
		desc: "comparison",
		in: `
			lss: 1 < 2
...
func TestResolve(t *testing.T) {
	testCases := []testCase{{
		...
		in: `
				...
				e2: int & 4.0/2.0
				`,
		out: `<0>{v1: 5e+11, v2: true, n1: 1, v5: 2, e1: 2.0, e2: _|_((int & (4.0 / 2.0)):conflicting values int and (4.0 / 2.0) (incompatible types int and float))}`,
	}, {
		desc: "inequality",
		in: `
			a: 1 != 2
		...
		out: `<0>{i1: 5, i2: 5, i3: [], i4: (!=2 & !=4), ` +
			...
			`e9: _|_((>"a" & <1):conflicting values >"a" and <1 (incompatible types string and number))}`,
	}, {
		...
		out: `<0>{` +
			...
			`e1: _|_((int & float):conflicting list element types: conflicting values int and float (incompatible types int and float))` +
			`}`,
	}, {
		...
		in: `
			a: "a" & 1
			`,
		out: `<0>{a: _|_(("a" & 1):conflicting values "a" and 1 (incompatible types string and int))}`,
	}, {
		desc: "structs",
		in: `
			a: t & { c: 5 }             // {c:5,d:15}
	...
	4)
		...
		out: `<0>{` +
			...
			`c1: (int & >=1 & <=5), ` +
			`c2: (<=5 & int & >=1), ` +
			`c3: _|_((string & >=1):conflicting values string and >=1 (incompatible types string and number)), ` +
			`c4: _|_(((>=1 & <=5) & string):conflicting values (>=1 & <=5) and string (incompatible types number and string)), ` +
			`s1: "e", ` +
			`s2: "ee", ` +
			`n1: (>=1 & <=2), ` +
			`n2: (int & >=1.1 & <=1.3), ` +
...
func TestFullEval(t *testing.T) {
	testCases := []testCase{{
		...
		in: `
				a: 8000.9
				a: 7080 | int`,
		out: `<0>{a: _|_((8000.9 & (int | int)):conflicting values 8000.9 and int (incompatible types float and int))}`, // TODO: fix repetition
	}, {
