...
func (c Config) newErrInstance(pos token.Pos, path string, err error) *build.Instance {
...
func (c *Config) absDirFromImportPath(pos token.Pos, p importPath) (absDir, name string, err errors.Error) {
	...
	switch hasPrefix := strings.HasPrefix(sub, c.Module); {
	case hasPrefix && len(p) == len(c.Module):
		absDir = c.ModuleRoot
