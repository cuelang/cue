...
type Config struct {
	...
	// BuildEnv defines boolean tags or key-value pairs to select files to build
	...
	//
	// The BuildEnv values are used to both select which files get included in a
	...
	//    field: x @tag(key)
	//
	// and BuildEnv value for which the name matches key, the field will be
	// modified to
	//
	//   field: x & "value"
	//
	...
	//
	// the BuildEnv entry "prod" sets the environment field to the value "prod".
	// This is equivalent to a BuildEnv entry of "env=prod".
	//
