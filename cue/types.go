...
// Path returns the path to this value from the root of a configuration.
//
...
func (v Value) Path() Path {
	...
	for i, f := range p {
		...
		case adt.DefinitionLabel, adt.HiddenDefinitionLabel, adt.HiddenLabel:
			a[i] = definitionSelector(f.SelectorString(v.idx.Runtime))

		case adt.StringLabel:
			a[i] = stringSelector(f.StringValue(v.idx.Runtime))
		}
	}
	return a
}
...
outer:
	for _, sel := range p {
		...
		var x *adt.Bottom
		if err, ok := sel.(pathError); ok {
			x = &adt.Bottom{Err: err.Error}
		} else {
			// TODO: better message.
			x = v.idx.mkErr(n, codeNotExist, "value %q not found", sel)
		}
		v := makeValue(v.idx, n)
