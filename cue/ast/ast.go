...
// NewStruct creates a struct from the given field arguments.
//
// A field argument is one of:
//
//	- a *Field describing a field in the struct;
//	- an *Ellipsis indicating that the struct is open (only valid as the last field);
//	- a *CommentGroup holding a comment on the following field;
//	- a Label or a string, holding the field's name,
//	   optionally followed by token.OPTION to indicate the field is optional
//	   and token.ISA to indicate the field is a definition, then followed by
//	   an Expr that holds the field's value.
//
